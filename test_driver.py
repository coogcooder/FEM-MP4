...
def build_cases():
    ...
    # --------------------------------------------------------
    # Case 1: Fully constrained, left edge 0, right edge 1
    # --------------------------------------------------------
    diffusivity = {'type': 'isotropic', 'value': 1.0}
    load_type = {'type': 'constant', 'value': 0.0}
    Constraints = np.array([
        [1, 1, 0.0], [4, 1, 0.0],
        [2, 1, 1.0], [3, 1, 1.0]
    ], dtype=float)
    cases.append(("Fully constrained (0–1)", Connectivity, Coord,
                  diffusivity, dim, dofs_per_node, EleType,
                  Constraints, load_type, NGPTS))

    # --------------------------------------------------------
    # Case 2: Left edge fixed (u=0), uniform volumetric source f=10
    # --------------------------------------------------------
    diffusivity = {'type': 'isotropic', 'value': 2.0}
    load_type = {'type': 'constant', 'value': 10.0}
    Constraints = np.array([
        [1, 1, 0.0], [4, 1, 0.0]
    ], dtype=float)
    cases.append(("Left fixed, uniform source f=10", Connectivity, Coord,
                  diffusivity, dim, dofs_per_node, EleType,
                  Constraints, load_type, NGPTS))

    # --------------------------------------------------------
    # Case 3: Spatially varying diffusivity and source term
    # --------------------------------------------------------
    def dfunc(x):
        return 1.0 + 2.0 * x[0]  # increases with x

    def ffunc(x):
        return 5.0 * (x[0] + x[1])  # position-dependent source

    diffusivity = {'type': 'callable', 'func': dfunc}
    load_type = {'type': 'callable', 'func': ffunc}
    Constraints = np.array([
        [1, 1, 0.0], [4, 1, 0.0]
    ], dtype=float)
    cases.append(("Spatially varying D(x) & f(x)", Connectivity, Coord,
                  diffusivity, dim, dofs_per_node, EleType,
                  Constraints, load_type, NGPTS))
...
def main():
    ...
    print("\nAll selected test cases completed successfully.\n")
    return results
